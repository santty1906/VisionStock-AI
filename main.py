...
from db import init_db, fetch_detections_paged, counts_by_label, totals, clear_inventory
...
from inventory_chatbot import build_reply
from vision_service import VisionService
from recognizer import resolve_device
...
def detections(
    ...
    return fetch_detections_paged(
        page=page,
        page_size=page_size,
        label=label,
        since_ts=since_iso,
        since_minutes=last_minutes,
    )
...
def chatbot_plan(payload: ChatbotRequest):
    detections = fetch_detections(limit=5000)
    if payload.window_days:
        now = datetime.now()
        cutoff = now - timedelta(days=int(payload.window_days))
        detections = [d for d in detections if d.get("ts", "") >= cutoff.isoformat(timespec="seconds")]
    reply = build_reply(payload.prompt, detections)
    return {"reply": reply}
